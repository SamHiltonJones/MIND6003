...
class PointCloudProcessor(Node):
    def __init__(self):
        ...
        self.publisher = self.create_publisher(String, 'matrix_topic', 10)  # Use String publisher
    ...
    def process_point_cloud(self, trans, output_file_name):
        ...
        matrix_str = np.array2string(final_transformation_matrix, separator=', ')
        self.publisher.publish(String(data=matrix_str))
        self.get_logger().info('Published transformation matrix as string to topic')


        camera_direction = final_transformation_matrix[:3, :3] @ np.array([1, 0, 0])
        filtered_points = [point for point in np.asarray(pcd.points) 
                        if self.is_point_in_cone(point, camera_offset_translation[2], np.deg2rad(140), np.deg2rad(103), 
